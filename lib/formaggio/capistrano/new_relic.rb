  ...
  namespace :newrelic do
    ...
    task :set do
      if new_relic_environment?
        run_locally "bundle exec rake formaggio:newrelic:set RAILS_ENV=#{fetch(:rails_env, 'staging')}"
      end
    end
    ...
    task :reset do
      if new_relic_environment?
        run_locally "bundle exec rake formaggio:newrelic:reset RAILS_ENV=#{fetch(:rails_env, 'staging')} "
      end
    end
  end
end
